...
%     EEG = pop_epoch(EEG, {conditions{cc}}, win_of_interest, 'newname', strcat(filename,'_',conditions{cc}), 'epochinfo', 'yes');
    ...
    %Group STD into STD1 (STD preceding DEV1) and STD2 (STD preceding DEV2)
    ...
    %Get indices from rejected STD (all standards)
    [EEG_STD_thresh, idx_removed] = pop_eegthresh(EEG_STD,1,elec ,rej_low, rej_high, win_of_interest(1), win_of_interest(2),0,1);
   
    %Get indices from kept STD
    std_good = setdiff(1:900,target_indices_std(idx_removed)); 
    
    %Create matrice that contains indices of all 3 first trials of each
    %block (30 blocks of 30 trials in total)
    ...
    if length(EEG_DEV1_thresh.event)>length(EEG_STD1_thresh.event)
       ...
       balance_number_of_STD(EEG,ntrials,std_good,target_indices1,begining_of_block,target_indices_std,idx_std1_rej)
    ...
    if length(EEG_DEV2_thresh.event)>length(EEG_STD2_thresh.event)
       ...
       %Apply function to balance number of STD trial to reach the number of DEV trials 
       balance_number_of_STD(EEG,ntrials,std_good,target_indices2,begining_of_block,target_indices_std,idx_std2_rej)
...
% STD 
%--------------------------------------------------------------
function [EEG_STD_ALL] = balance_number_of_STD(EEG,ntrials,std_good,target_indices,begining_of_block,target_indices_std,idx_std_rej)
